...
G_DEFINE_TYPE_WITH_CODE (IBusPortalContext, ibus_portal_context, IBUS_DBUS_TYPE_INPUT_CONTEXT_SKELETON,
                         G_IMPLEMENT_INTERFACE (IBUS_DBUS_TYPE_INPUT_CONTEXT, ibus_portal_context_iface_init));
                    ...
                    gpointer user_data)
{
    ...
    IBusPortalContext *portal_context = (IBusPortalContext *) g_dbus_method_invocation_get_user_data (invocation);
    IBusEngineDesc *desc;
    GError *error = NULL;
...
ibus_portal_context_iface_init (IBusDbusInputContextIface *iface)
{
    ...
    iface->handle_process_hand_writing_event = ibus_dbus_context_process_hand_writing_event;
    ...
    iface->handle_set_cursor_location_relative = ibus_dbus_context_set_cursor_location_relative;
...
ibus_portal_context_finalize (GObject *object)
{
    ...
    g_dbus_interface_skeleton_unexport (G_DBUS_INTERFACE_SKELETON (portal_context->service));
    g_dbus_interface_skeleton_unexport (G_DBUS_INTERFACE_SKELETON (portal_context));

    g_free (portal_context->owner);
    g_free (portal_context->object_path);
    g_object_unref (portal_context->service);

    g_signal_handlers_disconnect_by_func (portal_context->context, G_CALLBACK(portal_context_g_signal), portal_context);
...
ibus_portal_context_class_init (IBusPortalContextClass *klass)
{
  ...
  gobject_class->set_property  = (GObjectSetPropertyFunc) ibus_portal_context_set_property;
  gobject_class->get_property  = (GObjectGetPropertyFunc) ibus_portal_context_get_property;

  skeleton_class = G_DBUS_INTERFACE_SKELETON_CLASS(klass);
  skeleton_class->g_authorize_method = ibus_portal_context_g_authorize_method;

  ibus_dbus_input_context_override_properties (gobject_class, PROP_CONTENT_TYPE);
}
                         ...
                         IBusPortalContext *portal_context)
{
    ...
    if (!g_dbus_connection_emit_signal (g_dbus_interface_skeleton_get_connection (G_DBUS_INTERFACE_SKELETON (portal_context)),
...
ibus_portal_context_new (IBusInputContext *context,
                         ...
                         GError **error)
{
    IBusPortalContext *portal_context = g_object_new (ibus_portal_context_get_type (), NULL);

    g_signal_connect (context, "g-signal", G_CALLBACK(portal_context_g_signal), portal_context);

    portal_context->id = ++next_context_id;
    portal_context->context = g_object_ref (context);
    portal_context->owner = g_strdup (owner);
    portal_context->object_path = g_strdup_printf (IBUS_PATH_INPUT_CONTEXT, portal_context->id);
    portal_context->service = ibus_dbus_service_skeleton_new ();

    g_signal_connect (portal_context->service, "handle-destroy", G_CALLBACK (ibus_portal_context_handle_destroy), portal_context);

    if (!g_dbus_interface_skeleton_export (G_DBUS_INTERFACE_SKELETON (portal_context->service),
                                           connection, portal_context->object_path,
                                           error) ||
        !g_dbus_interface_skeleton_export (G_DBUS_INTERFACE_SKELETON (portal_context),
                                           connection, portal_context->object_path,
                                           error)) {
        g_object_unref (portal_context);
        return NULL;
...
G_DEFINE_TYPE_WITH_CODE (IBusPortal, ibus_portal, IBUS_DBUS_TYPE_PORTAL_SKELETON,
                         G_IMPLEMENT_INTERFACE (IBUS_DBUS_TYPE_PORTAL, ibus_portal_iface_init));
                           ...
                           GDBusMethodInvocation *invocation)
{
    ...
    context = ibus_bus_create_input_context_async_finish (ibus_bus, res, &error);
    if (context == NULL) {
        g_dbus_method_invocation_return_gerror (invocation, error);
        g_error_free (error);
        return;
    ...
    portal_context = ibus_portal_context_new (context,
                                              g_dbus_method_invocation_get_connection (invocation),
                                              g_dbus_method_invocation_get_sender (invocation), &error);
    ...
    ibus_dbus_portal_complete_create_input_context (IBUS_DBUS_PORTAL(ibus_portal),
...
{
    ibus_bus_create_input_context_async (ibus_bus,
                                         arg_client_name, -1,
                                         NULL,
...
{
    iface->handle_create_input_context = ibus_portal_handle_create_input_context;
}
                 ...
                 gpointer         user_data)
{
    ...
    if (!g_dbus_interface_skeleton_export (G_DBUS_INTERFACE_SKELETON (ibus_portal),
                                           connection,
                                           IBUS_PATH_IBUS,
                                           &error))
    {
